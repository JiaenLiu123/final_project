...
def get_and_clean_OCR(image, ocr_lang="fra", tesseract_config="--psm 12 --oem 1"):
    """
    Get & Clean OCR output
    :param image: image
    :return: OCR output
    """
    try:
        ocr_df = pytesseract.image_to_data(image, output_type="data.frame", lang=ocr_lang, config=tesseract_config)
...
def preprocessing(image):
    ...
    :param image: an image object in numpy array, PIL and OpenCV are supported
    :return: preprocessed data
    """

    # get OCR output
    inferernce_data = dict()
    clean_output = get_and_clean_OCR(image)
